...
public abstract class Game {
	...
	private static TraitType	winner			= TraitType.VIRUS;
	...
	public static TraitType getWinner() {
		return winner;
	}
	...
	public static void searchForOpponent() {
		// Waiting dialog
		final StandardMessageDialog message = new StandardMessageDialog(
				"Searching for an opponent...", 30);
	...
	private static void checkGameOver(MasterPackage pkg) {
		int totalHealthy = 0;
		for (Country c : Country.getCountryList()) {
			totalHealthy += c.getHealtyPeople();
		}

		if (totalHealthy == 0) {
			pkg.setGameOver(true);
			pkg.setWinner(TraitType.VIRUS);
			gameOver = true;
			winner = TraitType.VIRUS;
		} else if (turnsToCure == 0) {
			pkg.setGameOver(true);
			pkg.setWinner(TraitType.CURE);
			gameOver = true;
			winner = TraitType.CURE;
		}
	}
	...
	private static void endTurnMaster() {
		try {
			...
			pkg.setTemperateLevel(VirusStatistics
					.getTemperatureLevel(CountryClimateTemperature.TEMPERATE));
			...
			pkg.setMediterraneanLevel(VirusStatistics
					.getHumidityLevel(CountryClimateHumidity.MEDITERRANEAN));
	...
	private static void endTurnSlave() {
		try {
			...
			VirusStatistics.setTemperatureLevel(CountryClimateTemperature.HOT,
					response.getHotLevel());
			VirusStatistics.setTemperatureLevel(CountryClimateTemperature.COLD,
					response.getColdLevel());
			VirusStatistics.setTemperatureLevel(CountryClimateTemperature.TEMPERATE,
					response.getTemperateLevel());

			// Update humidity data
			VirusStatistics.setHumidityLevel(CountryClimateHumidity.ARID, response.getAridLevel());
			VirusStatistics.setHumidityLevel(CountryClimateHumidity.TROPICAL,
					response.getTropicalLevel());
			VirusStatistics.setHumidityLevel(CountryClimateHumidity.MEDITERRANEAN,
					response.getMediterraneanLevel());
	...
	private static void updateTurn(SlavePackage slave) {
		...
		VirusStatistics.setInfectiousness(VirusStatistics.getInfectiousnessDelta()
				+ slave.getInfectiousnessDelta());
		VirusStatistics.setDeadliness(VirusStatistics.getDeadlinessDelta()
				+ slave.getDeadlinessDelta());
		VirusStatistics.setNotoriety(VirusStatistics.getNotorietyDelta()
				+ slave.getNotorietyDelta());
		...
		for (Country c : Country.getCountryList()) {
			if (c.getState() == CountryState.INFECTED) {
				...
				double specific = (VirusStatistics.isLivestockTransmission() && c.getType() == CountryType.RURAL)
						|| (VirusStatistics.isPlagueTransmission() && c.getType() == CountryType.INDUSTRIAL) ? 0.05
						: 0.0;
				...
				int newInfected = Math.min(c.getHealtyPeople(),
						(int) Math.ceil(c.getInfectedPeople() * bonus * penalty) * 2);
		...
		if (turnNumber >= TURNS_TO_DEATH) {
			for (Country c : Country.getCountryList()) {
				int newDead = Math.min(c.getInfectedPeople(),
						(int) Math.ceil(c.getInfectedPeople() * VirusStatistics.getDeadliness()));
		...
		for (Country c : Country.getCountryList()) {
			if (c.getInfectedPeople() == 0 && c.getAlivePeople() > 0) {
				double penalty = VirusStatistics.getTemperatureBonus(c.getTemperature())
						* VirusStatistics.getHumidityBonus(c.getHumidity());
				if (penalty == 0.0)
					continue;
				double prob = 0.0;
		...
		for (Country c : Country.getCountryList()) {
			...
			if (c.getDeadPeople() / (double) c.getTotalPeople() > c.getThresholds()
					.getDeadThreshold()
					|| c.getInfectedPeople() / (double) c.getTotalPeople() > c.getThresholds()
							.getInfectedThreshold()) {
