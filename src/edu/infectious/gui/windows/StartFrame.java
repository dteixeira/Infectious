...
public class StartFrame extends JFrame {
	...
	private void setupBackground() {
		try {
			BufferedImage toResize = ImageIO.read(new File(BACKGROUND_FILENAME));
			background = new BufferedImage(screenSize.width, screenSize.height, toResize.getType());
			Graphics2D g2d = background.createGraphics();
			g2d.drawImage(toResize, 0, 0, screenSize.width, screenSize.height, null);
			g2d.dispose();
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(0);
	...
	private void setupButtons() {
		...
		startButton = new Button(marginX, marginY, BUTTON_WIDTH, BUTTON_HEIGHT, BUTTON_LINE_WIDTH,
				"Find Opponent", BUTTON_COLOR, BUTTON_LINE_COLOR);
		quitButton = new Button(marginX + 20 + BUTTON_WIDTH, marginY, BUTTON_WIDTH, BUTTON_HEIGHT,
				BUTTON_LINE_WIDTH, "Exit", BUTTON_COLOR, BUTTON_LINE_COLOR);
	}
